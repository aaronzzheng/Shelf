#!/bin/bash
# Builds Shelf.app — an unsigned-except-ad-hoc bundle you can run locally
...
if [ "$UNIVERSAL" = 1 ]; then
  ...
  BIN=$(swift build -c release --arch arm64 --arch x86_64 --show-bin-path)/Shelf
else
...
if [ "$INSTALL" = 1 ]; then
  ...
  osascript -e 'quit app "Shelf"' >/dev/null 2>&1 || true
  for _ in $(seq 1 20); do
    pgrep -f "Shelf.app/Contents/MacOS/Shelf" >/dev/null 2>&1 || break
    sleep 0.25
  done
